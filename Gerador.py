        ...
        if title == type and se_count == index:
            for memo_row in range(1, memo.max_row + 1):
                ...
                if conferencia_value in white_list and qtd_validador(memo_row, se):
                    ...
                    complete_cells(planilha_preco_row,
                                   memo_row, row_count, taxes)
...
def get_sum_formula(first_planilha_preco_row, last_planilha_preco_row, column):
    formula = f'=SUM({column}{first_planilha_preco_row+1}:{column}{last_planilha_preco_row})'
    return formula
...
def make_titles(names_se):
    ...
    for se in names_se:
        ...
        if names_se.index(se) == 0:  # preencher caso for a primeira SE
            planilha_preco.cell(
                row=4, column=1, value=str(se))  # Nome do título
            # Nome da primeira SE
            planilha_preco.cell(row=8, column=2, value=str(se))
            planilha_preco.cell(
                row=8, column=1, value=int(names_se.index(se) + 1))
            ...
            for row in range(total_row, total_row + 6):
                for column in range(1, planilha_preco.max_column + 1):
                    ...
                    new_cell = planilha_preco.cell(
                        row=row, column=column, value="")
                    new_cell._style = copy(copy_cell._style)

                    if row == total_row and column == 2:  # preenche o nome da SE
                        planilha_preco.cell(
                            row=row, column=column, value=str(se))

                    elif column == 2:
                        new_cell = planilha_preco.cell(
                            row=row, column=column, value=copy_cell.value)  # preenche a descrição

                    if row == total_row and column == 1:
                        planilha_preco.cell(row=row, column=column, value=int(
                            names_se.index(se) + 1))  # preenche o primeiro item
    ...
    for planilha_preco_row in range(1, 1000):
        ...
        if planilha_preco[f'B{planilha_preco_row}'].value == 'ELÉTRICA' and se_count == se_names.index(se) + 1:
            ...
            for planilha_preco_column in range(1, planilha_preco.max_column + 1):
                ...
                new_cell = planilha_preco.cell(
                    row=planilha_preco_row + 1, column=planilha_preco_column, value="")
                new_cell._style = copy(copy_cell._style)

            planilha_preco.cell(row=planilha_preco_row + 1,
                                column=2, value="EQUIPAMENTOS DE PÁTIO")
    ...
    for planilha_preco_row in range(1, 1000):
        ...
        if planilha_preco[f'B{planilha_preco_row}'].value == 'CIVIL' and se_count == se_names.index(se) + 1:
            ...
            for planilha_preco_column in range(1, planilha_preco.max_column + 1):
                copy_cell = styles[f'{get_column_letter(planilha_preco_column)}4']
                new_cell = planilha_preco.cell(
                    row=planilha_preco_row, column=planilha_preco_column, value="")
                new_cell._style = copy(copy_cell._style)

            planilha_preco.cell(row=planilha_preco_row,
                                column=2, value="CASA DE COMANDO")

    make_casa_itens(se, "Cubículos", "CUBÍCULOS DE MÉDIA TENSÃO")
    make_casa_itens(se, "Proteção, medição e controle",
...
def make_casa_itens(se, memo_value, planilha_precos_title):
    ...
    for planilha_preco_row in range(1, 1000):
        ...
        if planilha_preco[f'B{planilha_preco_row}'].value == 'CIVIL' and se_count == se_names.index(se) + 1:
            ...
            planilha_preco.cell(row=planilha_preco_row,
                                column=2, value=planilha_precos_title)
...
def make_other_titles():
    ...
    for title in titles:
        ...
        for planilha_preco_column in range(1, planilha_preco.max_column + 1):
            ...
            new_cell = planilha_preco.cell(
                row=title_row, column=planilha_preco_column, value=None)
            new_cell._style = copy(copy_cell._style)
